...
function App() {
  ...
  // ** Counter : Data 전달 (props,event 전달)
  ...
  // -> 랜더링 할때마다 호출 됨
  useEffect(()=>{
    // console.log(`** useEffect Test3) 두번째인자 없음 count=${count}, text=${text}`)
});

  // 4) LifeCycle 제어2, 두번째 인자가 빈배열 인경우
  //  -> Mount 제어
  //  -> useEffect 에 빈 배열을 전달하면 마운트 시점에만 콜백함수 실행
  //    ( 처음 한번만 실행됨 확인 -> 그러므로 Mount 제어에 이용)
  useEffect(()=>{
    // console.log(`** useEffect Test4) 빈배열 count=${count}, text=${text}`)
},[]);

  // 5) LifeCycle 제어3,
  //  -> Update(리랜더링) 시에만 호출
  //  -> 3)의 경우에서 4)의 경우만 제외해서 사용
  ...
  useEffect(()=>{
    if (!didMount.current) {
      ...
      // console.log(`** useEffect Test5) 빈배열 count=${count}, text=${text}`)
    }
  })
